  ...
  class Jasperserver
    ...
    def run_report(filename, &block)
      ...
      response2 = RestClient.get(
        URI.join(Global.jasperserver.url + '/', "rest_v2/reports/reports/#{self.get_report}.#{self.get_format}?#{URI.encode_www_form(self.get_params)}").to_s,
        {:cookies => @cookie }
      )
    ...
    def login
      ...
      if @cookie.nil?
        ...
        URI.join(Global.jasperserver.url + '/', 'rest/login').to_s,
          {
            j_username: Global.jasperserver.username,
            j_password: Global.jasperserver.password
          }
        )
